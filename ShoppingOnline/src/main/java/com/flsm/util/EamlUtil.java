...
public class EamlUtil {
	/*
	    1 发件人的 邮箱 和 密码
	    2 某些邮箱服务器为了增加邮箱本身密码的安全性，给 SMTP 客户端设置了独立密码（有的邮箱称为"授权码"）, 
	    3 对于开启了独立密码的邮箱, 这里的邮箱密码必需使用这个独立密码（授权码）。
	 */
	private String sendEmail="1772213509@qq.com";
	private String password="njhnofunjbsdcdbh";
	...
	public  EamlUtil(String receiveEmail, String subject, String content)
	{
	...
	private MimeMessage createMessage(Session session, String subject, String content) throws Exception
	{
		...
		// 设置发件人
		message.setFrom(new InternetAddress(sendEmail, "橘子分享", "UTF-8")); 
		// 收件人（可以增加多个收件人、抄送、密送）
		message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiveEmail, "Latiny 用户", "UTF-8"));     
		// 邮件主题
